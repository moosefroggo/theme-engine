...
function ProjectCardDownload({
  ...
  const cardBg = seed.tokens["--background"]
    ? `hsl(${seed.tokens["--background"]})`
    : bg;
  const cardFg = seed.tokens["--foreground"]
    ? `hsl(${seed.tokens["--foreground"]})`
    : fg;
  const cardMuted = seed.tokens["--muted-foreground"]
